...
contract AbstractERC20 {
    ...
    event BountInvestorTransfer(address indexed _from, address indexed _to, uint256 _value);
    event TeamAndAdvisorsAndFoundersTransfer(address indexed _from, address indexed _to, uint256 _value);
// Event triggered when pre-ICO or ICO tokens were burned.
    event Burned(address _address, uint256 _amount);
    event AllocatedTokensToBounty(address _owner, address _investor, uint256 _tokenAmount);
    event AllocatedTokensToTeamAndAdvisorsAndFounders(address _owner, address _investor, uint256 _tokenAmount);
}
...
contract LQCoin4 is Ownable, AbstractERC20 {
    ...
    uint public tokensIssuedITO   = 0;
       // The time after which Liquid tokens become transferable.
    // Current value is July 24, 2018 8:30 PM.
    uint256 becomesBountyTransferable = 1532444400;
    
    uint256 becomesTeamAndAdvisorsAndFounderTransferable = 1532736000;
    mapping (address => uint256) internal balances;
     /// The transfer allowances
    mapping (address => mapping (address => uint256)) internal allowed;
    mapping (address => bool) public isBountyInvestor;
    mapping (address => bool) public isTeamAndAdvisorsAndFounderInvestor;
...
// End - 07/26/2018 @ 5:3pm (IST)
 ...
 /// manually send tokens to Bountyinvestor
    function allocateTokensToBountyInvestors(address _to, uint256 _value) public onlyOwner returns (bool success) {
        require(_to != address(0x0));
        require(_value > 0);
        balances[owner] = balances[owner].sub(_value);
        balances[_to] = balances[_to].add(_value);

        isBountyInvestor[_to] = true;
        emit AllocatedTokensToBounty(msg.sender, _to, _value);
        return true;
    }

    /// manually send tokens to team, advisors, founders 
    function allocateTokensToTeamAndAdvisorsAndFounders(address _to, uint256 _value) public onlyOwner returns (bool success) {
        require(_to != address(0x0));
        require(_value > 0);
        // SafeMath.sub will throw if there is not enough balance.
        balances[owner] = balances[owner].sub(_value);
        balances[_to] = balances[_to].add(_value);
        isTeamAndAdvisorsAndFounderInvestor[_to] = true;
        emit AllocatedTokensToTeamAndAdvisorsAndFounders(msg.sender, _to, _value);
        return true;
    }
   ...
   * @dev Check balance of given account address
    ...
    */
    function transfer(address to, uint256 value) public returns (bool) {
        ...
        if(isBountyInvestor[msg.sender]){
           require(now >= becomesBountyTransferable);
        balances[msg.sender] = balances[msg.sender].sub(value);
        balances[to] = balances[to].add(value);
        emit BountInvestorTransfer(msg.sender, to, value); 
        }
        
        if(isTeamAndAdvisorsAndFounderInvestor[msg.sender]){
           require(now >=becomesTeamAndAdvisorsAndFounderTransferable );
        balances[msg.sender] = balances[msg.sender].sub(value);
        balances[to] = balances[to].add(value);
        emit TeamAndAdvisorsAndFoundersTransfer(msg.sender, to, value); 
        }
        balances[msg.sender] = balances[msg.sender].sub(value);
        balances[to] = balances[to].add(value);
    ...
    */
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        ...
        if(isTeamAndAdvisorsAndFounderInvestor[from]){
           require(now >=becomesTeamAndAdvisorsAndFounderTransferable );
        balances[from] = balances[from].sub(value);
        balances[to] = balances[to].add(value);
        emit TeamAndAdvisorsAndFoundersTransfer(msg.sender, to, value); 
...
function AddWhitelist(address addr) public onlyOwner  {
   ...
   */
  function addAddressesToWhitelist(address[] _addrs) public onlyOwner
  {
...
/* Fallback */
  ...
  function ()  public payable {
    ...
    require(whitelist[msg.sender]);a
 ...
 function GetBouns(uint256 amount) internal view returns (uint) {
     uint Bonustokens=0;
        uint Calcuatedtokens=0;
      Bonustokens = amount * tokensPerKEther / 10**uint(18 - decimals + 3);
      //Bonus Available 50,000 5%; 
              if(Bonustokens == 50000){
                
                   Calcuatedtokens = Bonustokens.mul(100 + 5) / 100;
              }
             //Bonus Available  100,000 10%;
              if(Bonustokens== 100000){
                  
                  
                   Calcuatedtokens = Bonustokens.mul(100 + 10) / 100;
              }
               //Bonus Available  200,000 15%
              if(Bonustokens == 200000){
                  
                  
                   Calcuatedtokens = Bonustokens.mul(100 + 15) / 100;
              }
              
              Bonustokens = Calcuatedtokens;
